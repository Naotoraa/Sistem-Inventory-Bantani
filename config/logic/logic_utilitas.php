...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $biaya      = str_replace('.', '', $_POST['biaya'] ?? '0');
    $keterangan = $_POST['keterangan'] ?? '';

    if ($action === 'insert') {
        $stmt = $conn->prepare("
            INSERT INTO utilitas 
            (id_utilitas, pembayaran, tanggal, biaya, keterangan) 
            VALUES (?, ?, ?, ?, ?)
        ");
        ...
        if ($stmt_check->num_rows > 0) {
            $stmt = $conn->prepare("
                UPDATE utilitas 
                SET pembayaran=?, tanggal=?, biaya=?, keterangan=? 
                WHERE id_utilitas=?
            ");
