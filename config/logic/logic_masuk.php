...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST['action'])) {
    ...
    if ($_POST['action'] == 'update') {
        ...
        // UPDATE
        $sql = "UPDATE barang_masuk SET id_barang = ?, kategori = ?, qty = ?, tanggal_masuk = ? WHERE id = ?";
        ...
        // INSERT
        $sql = "INSERT INTO barang_masuk (id_barang, kategori, qty, tanggal_masuk) VALUES (?, ?, ?, ?)";
        $stmt = $conn->prepare($sql);
...
if (!empty($bulan)) {
    $conditions[] = "DATE_FORMAT(bm.tanggal_masuk, '%Y-%m') = '" . $conn->real_escape_string($bulan) . "'";
}
