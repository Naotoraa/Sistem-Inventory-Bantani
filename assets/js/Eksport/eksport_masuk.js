  ...
  document.querySelector(".pdf").addEventListener("click", async function () {
    ...
    if (!hasValidData) {
      Swal.fire({
        ...
        text: "Data barang masuk untuk periode ini kosong.",
        confirmButtonText: "OK",
      });
      return;
    }
    ...
    doc.setFontSize(16);
    doc.text("LAPORAN BARANG MASUK", doc.internal.pageSize.getWidth() / 2, 40, {
      align: "center",
    });
    ...
    // === SIMPAN PDF ===
    const fileName = `Laporan Barang Masuk - ${bulanText.replace(
      /\s+/g,
      "_"
    )}.pdf`;
    doc.save(fileName);
